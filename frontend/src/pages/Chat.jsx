        ...
        if (data.messages && data.messages.length > 0) {
          ...
          setMessages(prev => {
            ...
            data.messages.forEach(msg => {
              const msgSenderId = (typeof msg.sender === 'object' ? msg.sender._id : msg.sender).toString();
              const isOwn = msgSenderId === myUserId;

              const existingIdx = updated.findIndex(m => m._id === msg._id ||
                (isOwn && m.isOwn && m._id?.startsWith('temp-') && (
                  (msg.messageType === 'text' && m.message?.trim() === msg.content?.trim()) ||
                  (msg.messageType !== 'text' && m.fileUrl === msg.fileUrl)
                )));
              ...
              const formattedMsg = {
                ...
                fileUrl: msg.fileUrl,
                sender: isOwn ? myUsername : partnerUsername,
              ...
              if (existingIdx > -1) {
                // Update existing message (e.g., if it was deleted or just synced from temp)
                if (JSON.stringify(updated[existingIdx]) !== JSON.stringify(formattedMsg)) {
                  updated[existingIdx] = formattedMsg;
                  hasChanges = true;
                }
              } else {
                // Add new message
